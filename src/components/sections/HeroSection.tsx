    ...
    <section style={{ background: '#fff', padding: isMobile ? '32px 5% 40px' : '80px 6% 64px', borderBottom: '1px solid var(--border)', overflow: 'hidden' }}>
      <div style={{ maxWidth: '1200px', margin: '0 auto', display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: isMobile ? '32px' : '64px', alignItems: 'center' }}>
        ...
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
          ...
          <h1 style={{ fontSize: isMobile ? '44px' : 'clamp(40px,6vw,72px)', fontWeight: 800, letterSpacing: '-2px', color: 'var(--ink)', lineHeight: 1, marginBottom: '4px' }}>
            {s.hero_name}
          </h1>
          <h2 style={{ fontSize: isMobile ? '40px' : 'clamp(36px,5.5vw,66px)', fontWeight: 800, letterSpacing: '-2px', color: 'var(--coral)', lineHeight: 1, marginBottom: '18px' }}>
            {s.hero_title}
          </h2>
          <div style={{ width: '40px', height: '3px', background: 'var(--coral)', borderRadius: '2px', marginBottom: '18px' }} />
          <p style={{ fontSize: isMobile ? '16px' : 'clamp(16px,1.8vw,20px)', fontFamily: 'var(--font-playfair)', fontStyle: 'italic', color: 'var(--muted)', marginBottom: '14px', lineHeight: 1.5 }}>
            "{s.hero_quote}"
          </p>
          <p style={{ fontSize: '14px', color: 'var(--muted2)', lineHeight: 1.8, marginBottom: '24px' }}>
            {s.hero_description}
          </p>

          <div style={{ display: 'flex', flexDirection: isMobile ? 'column' : 'row', gap: '10px', flexWrap: 'wrap', marginBottom: '32px' }}>
            <Link href="/booking" style={{ padding: '13px 24px', background: 'var(--coral)', color: '#fff', borderRadius: '50px', fontSize: '13px', fontWeight: 700, textDecoration: 'none', boxShadow: '0 4px 20px rgba(240,99,90,0.35)', textAlign: 'center', display: 'block' }}>
              Book Appointment
            </Link>
            <Link href="/services" style={{ padding: '13px 24px', background: 'transparent', color: 'var(--ink)', borderRadius: '50px', fontSize: '13px', fontWeight: 700, textDecoration: 'none', border: '1.5px solid var(--border)', textAlign: 'center', display: 'block' }}>
              View Services
            </Link>
            <a href={`https://wa.me/${s.contact_whatsapp}`} target="_blank" rel="noopener"
              style={{ padding: '13px 24px', background: '#25D366', color: '#fff', borderRadius: '50px', fontSize: '13px', fontWeight: 700, textDecoration: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px' }}>
              💬 WhatsApp
            </a>
          </div>
            ...
            <div style={{ background: 'linear-gradient(155deg, var(--blush) 0%, #fff 100%)', borderRadius: '24px', padding: '28px', border: '1px solid var(--blush3)', boxShadow: '0 20px 60px rgba(240,99,90,0.12)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '14px', marginBottom: '16px', padding: '14px', background: '#fff', borderRadius: '12px', boxShadow: '0 2px 12px rgba(0,0,0,0.06)' }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: '11px', color: 'var(--muted2)', marginTop: '3px' }}>Travels to venue · {s.hero_availability}</div>
                </div>
              </div>
