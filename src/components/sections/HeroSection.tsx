...
export default function HeroSection() {
  ...
  return (
    <section style={{ background: '#fff', padding: '80px 6% 64px', borderBottom: '1px solid var(--border)', overflow: 'hidden' }}>
      <div className="hero-section-grid" style={{ maxWidth: '1200px', margin: '0 auto', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 480px), 1fr))', gap: '40px', alignItems: 'center' }}>
        ...
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
          ...
          <p style={{ fontSize: 'clamp(16px,1.8vw,20px)', fontFamily: 'var(--font-playfair)', fontStyle: 'italic', color: 'var(--muted)', marginBottom: '14px', lineHeight: 1.5 }}>
            "{s.hero_quote}"
          </p>
          ...
          <div className="hero-buttons" style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', marginBottom: '32px' }}>
            <Link href="/booking" style={{ padding: '13px 24px', background: 'var(--coral)', color: '#fff', borderRadius: '50px', fontSize: '13px', fontWeight: 700, textDecoration: 'none', boxShadow: '0 4px 20px rgba(240,99,90,0.35)', transition: 'all 0.2s', display: 'inline-block', textAlign: 'center' }}>
              Book Appointment
            </Link>
            <Link href="/services" style={{ padding: '13px 24px', background: 'transparent', color: 'var(--ink)', borderRadius: '50px', fontSize: '13px', fontWeight: 700, textDecoration: 'none', border: '1.5px solid var(--border)', transition: 'all 0.2s', display: 'inline-block', textAlign: 'center' }}>
              View Services
            </Link>
          ...
          <div className="hero-stats" style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: '8px' }}>
            {stats.map(k => (
        ...
        {/* Right card */}
        <motion.div className="hero-right-card" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6, delay: 0.15 }}>
