      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '64px', alignItems: 'center' }}>
        ...
        <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}
          ...
          <div style={{ borderRadius: '20px', overflow: 'hidden', aspectRatio: '3/4', background: 'linear-gradient(155deg, var(--coral) 0%, var(--blush3) 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            {s.about_photo_url ? (
              <img src={s.about_photo_url} alt="Sri Pujitha — Makeup Artist" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
            ) : (
          ...
          {/* Floating badge */}
          <div style={{ position: 'absolute', bottom: '24px', left: '-20px', background: '#fff', borderRadius: '12px', padding: '12px 16px', boxShadow: '0 8px 32px rgba(0,0,0,0.12)', display: 'flex', alignItems: 'center', gap: '10px' }}>
        ...
        <motion.div initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, delay: 0.1 }}>
          ...
          <h2 style={{ fontSize: 'clamp(28px,4vw,48px)', fontWeight: 800, letterSpacing: '-1px', color: 'var(--ink)', lineHeight: 1.1, marginBottom: '20px' }}>
          ...
          <p style={{ fontSize: '14px', color: 'var(--muted)', lineHeight: 1.85, marginBottom: '28px' }}>{s.about_bio}</p>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '12px', marginBottom: '28px' }}>
            {[
              ...
              <div key={k.l} style={{ background: 'var(--blush)', border: '1px solid var(--blush3)', borderRadius: '10px', padding: '14px 16px' }}>
                <div style={{ fontSize: '22px', fontWeight: 800, color: 'var(--coral)', letterSpacing: '-0.5px', lineHeight: 1 }}>{k.n}</div>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
            {tags.map(tag => (
              <span key={tag} style={{ padding: '6px 14px', background: '#fff', border: '1.5px solid var(--blush3)', borderRadius: '50px', fontSize: '11px', fontWeight: 600, color: 'var(--coral)' }}>
                {tag}
              </span>
            ))}
          </div>
